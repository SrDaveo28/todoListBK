import { render, addNavbarMobile, roleValidatorMobile } from '../others/utils.js';
import { tareasAsignadasMobile, navbarMobile } from '../views/templates.js';
import { getTareasAll } from '../apis/bdCall.js';
import { updateTarea } from '../apis/bdPut.js';

export function tareasAsignadasMob(view, model) {
    view = {};

    view.navbarMobile = navbarMobile(model.user);
    view.tareasAsignadasMobile = tareasAsignadasMobile();

    render(view, true);


    document.getElementById('tareasAsigMobile').innerHTML = `
    ...
    function frontCharge(tareas) {
        ...
        tareasFilter.forEach((tarea, index) => {
            if (tarea.acciones === "asignado") {
                valuesAsig += `<li class="list-group-item">${tarea.tarea} <br> <strong>Funcionario:</strong> ${tarea.ticket.asignado_user}<span class="icon deleteButton" id="${tarea.ticket.id}"><i
                class="fas fa-trash mt-3"></i></span></li>`;

            } else if (tarea.acciones === "enProceso") {
                valuesProcess += `<li class="list-group-item">${tarea.tarea} <br> <strong>Funcionario:</strong> ${tarea.ticket.asignado_user}<span class="icon deleteButton" id="${tarea.ticket.id}"><i
                class="fas fa-trash mt-3"></i></span></li>`;

            } else if (tarea.acciones === "Realizado") {
                valuesRealiz += `<li class="list-group-item">${tarea.tarea} <br> <strong>Funcionario:</strong> ${tarea.ticket.asignado_user}<span class="icon deleteButton" id="${tarea.ticket.id}"><i
                class="fas fa-trash mt-3"></i></span></li>`;
        ...
        document.getElementById('tareasAsigMobile').innerHTML = /* html */`
        <div class="row">
                    <div class="col">
                        <div class="wrapper">
